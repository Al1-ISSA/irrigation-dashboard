...
export default function LightController() {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      const motionRequest = axios.post("/api/light", {
        light: data.state,
      });
      ...
      toast.promise(motionRequest, {
        loading: "Please Wait",
        success: () => {
          return "Light state updated successfully"
        },
        error: (err) => {
          if (err.response) {
      ...
      toast.message("Light", {
        description: "Failed to update light state",
      });
    }
  }
  return (
      ...
      <Form {...form}>
        <form
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Light State</FormLabel>
                <Select
